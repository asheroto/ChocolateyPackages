...
$packageName = "fxsound"
$FILE_URL = 'https://download.fxsound.com/fxsoundlatest'
$downloadPath = '.\fxsound-temp.exe'
$nuspecPath = '.\fxsound.nuspec'
...
try {
    ...
    if ($productVersion -match '^\d+(\.\d+){1,3}$' -and $nuspecVersion -match '^\d+(\.\d+){1,3}$') {
        ...
        if ([version]$productVersion -gt [version]$nuspecVersion -or $newChecksum -ne $currentChecksum) {
            ...
            # Move the downloaded file to the tools directory and rename as fxsound_setup.exe
            Move-Item $downloadPath -Destination '.\tools\fxsound_setup.exe' -Force
        } else {
